        ...
        <section className='banner'>
            ...
            <ul className="slides-container">
                {bannerImages.map(function(img,index) {
                     ...
                     <img alt={'img' + index} className='slide2' src={img} style={{display: img + index === bannerImages[index] + activeSlide ? 'block' : 'none'}} />
                     </li>;
                 })}
             </ul>
             <ul className="pagination-container">
                 {bannerImages.map(function(dot, index) {
                     return <li className='dot' key={dot + index} onClick={() =>
                         setActiveSlide(index)} style={{backgroundColor: dot + index === dot + activeSlide ? 'black' : 'gray'}}></li>;
                 })}
             </ul>
         </section>
     );
